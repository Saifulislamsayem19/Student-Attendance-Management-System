...
class Student:
    def __init__(self, root):
        ...
        dep_combo["values"] = ("Select Department", "CSE", "EEE", "Civil")
        dep_combo.current(0)
        ...
        # Serial_No
        studentId_label = Label(class_Student_frame, text="Serial-No:", font=("verdana", 12, "bold"), fg="navyblue",
                                bg="white")
        ...
        student_reg_label = Label(class_Student_frame, text="Reg-No:", font=("verdana", 12, "bold"), fg="navyblue",
                                  bg="white")
        student_reg_label.grid(row=1, column=0, padx=5, pady=5, sticky=W)

        student_reg_entry = ttk.Entry(class_Student_frame, textvariable=self.var_reg, width=15,
                                      font=("verdana", 12, "bold"))
        student_reg_entry.grid(row=1, column=1, padx=5, pady=5, sticky=W)
        ...
        self.student_table = ttk.Treeview(table_frame, column=(
            "ID", "Reg", "Roll-No", "Name", "Dep", "Course", "Year", "Sem", "Gender", "DOB", "Mob-No", "Address",
            "Email",
        ...
        self.student_table.heading("Reg", text="Reg")
    ...
    def add_data(self):
        if self.var_dep.get() == "Select Department" or self.var_course.get == "Select Course" or self.var_year.get() == "Select Year" or \
                ...
                self.var_reg.get() == "" or self.var_roll.get() == "" or self.var_gender.get() == "" or self.var_dob.get() == "" or \
                self.var_email.get() == "" or self.var_mob.get() == "" or self.var_address.get() == "" or self.var_teacher.get() == "":
        ...
        # Reg = self.var_reg.get()
        # Gender = self.var_gender.get()
        # DOB = self.var_dob.get()
        # Mobile_No = self.var_mob.get()
        ...
        #
        # row = [Student_ID, '',Reg, '',Roll_No, '',Name, '',Department, '',Course, '',Year, '',Semester, '',Gender, '',DOB, '',Mobile_No, '',Address, '',Email, '',Teacher_Name, '',PhotoSample]
        #
        # with open('Student_details\Students_details.csv', 'a+') as csvFile:
